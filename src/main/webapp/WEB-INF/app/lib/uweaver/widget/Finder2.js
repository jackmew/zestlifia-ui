...
define(['underscore', 'jquery', 'uweaver/lang',
    ...
    var Finder = declare(Base, {
        ...
        onFormExecute: function(event) {
            var command = event.data.command;
            this.perform(   command);
        },

        onToolbarPerform: function (event) {
            var command = event.data.command;
            this.perform(command);
        },
    ...
    function prepare() {
        ...
        cache.$list = this.$gear('list');
        cache.$list.css('display', 'table');
        cache.$list.css('cursor', 'pointer');
        cache.$page = this.$gear("page");
        cache.$pages = this.$gear("pages");
        cache.$total = this.$gear("total");
        cache.$criteria = this.$gear('criteria');
        ...
        this.listenTo(form, 'fire', _.bind(this.onFormExecute, this));
        cache.form = form;
        ...
        _.each($toolbars, function($toolbar) {
            ...
            this.listenTo(toolbar, 'fire', _.bind(this.onToolbarPerform, this));
